    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div
            className={`relative flex items-center justify-between rounded-full px-4 sm:px-6 py-2.5 transition-[background,border-color,box-shadow,color] duration-300 ${
              isScrolled
                ? 'glass-navbar-dark text-white'
                : 'glass-navbar-light text-[#0F172A]'
            }`}
          >
            {/* Top edge specular glass highlight */}
            <div
              className={`absolute inset-x-8 top-0 h-[1px] bg-gradient-to-r from-transparent ${
                isScrolled ? 'via-white/30' : 'via-white/90'
            ...
            >
              <Logo variant="icon" isDarkBg={isScrolled} className="w-8 h-8" />
              <div className="flex flex-col">
                <div className="flex items-center gap-1.5">
                  <span className={`font-anton uppercase tracking-wider text-xl sm:text-2xl leading-none ${isScrolled ? 'text-white' : 'text-[#0D52BC]'}`}>
                    ISLAM
                  </span>
            ...
            <div
              className={`hidden md:flex items-center gap-1 rounded-full p-1 transition-[background-color,border-color,box-shadow] duration-200 ${
                isScrolled ? 'glass-inner-dark' : 'glass-inner-light'
              }`}
            >
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className={`flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-medium uppercase tracking-wider transition-[background-color,border-color,color,transform] duration-150 cursor-pointer active:scale-95 ${
                  isScrolled
                    ? 'bg-white/10 text-white hover:bg-white/20 border border-white/10'
                    : 'bg-white/80 text-[#18181B] hover:bg-white border border-white/90 shadow-xs'
                }`}
              >
              ...
              <div className={`h-4 w-px mx-1 ${isScrolled ? 'bg-white/10' : 'bg-black/10'}`} />
              ...
              <nav className="flex items-center space-x-1">
                {navLinks.slice(0, 4).map((link) => (
                  <a
                    key={link.href}
                    href={link.href}
                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-[color,background-color,transform] duration-150 active:scale-95 ${
                      isScrolled
                        ? 'text-zinc-300 hover:text-white hover:bg-white/10'
                        : 'text-zinc-700 hover:text-black hover:bg-white/70'
                    }`}
                  >
                    {link.label}
                  </a>
            ...
            <div className="flex items-center gap-2">
              <a
                href="#how-it-works"
                className={`w-9 h-9 rounded-full flex items-center justify-center transition-[background-color,color,border-color,transform] duration-150 cursor-pointer active:scale-90 ${
                  isScrolled
                    ? 'glass-inner-dark text-white hover:bg-white/15'
                    : 'glass-inner-light text-[#0F172A] hover:bg-white/90'
                }`}
              ...
              <button
                type="button"
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className={`md:hidden p-2 rounded-full transition-[background-color,border-color,transform] duration-150 cursor-pointer active:scale-90 ${
                  isScrolled
                    ? 'glass-inner-dark text-white'
                    : 'glass-inner-light text-[#0F172A]'
                }`}
