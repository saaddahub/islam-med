...
export const Hero: React.FC<HeroProps> = ({ onOpenAppointment }) => {
  ...
  return (
    <section id="hero" className="relative min-h-[100dvh] w-full flex flex-col justify-between overflow-hidden bg-[#F0F4FA] pt-28 pb-12 sm:pt-32 sm:pb-16">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#F0F4FA]/95 via-[#F0F4FA]/75 to-transparent sm:w-2/3" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#F0F4FA] via-transparent to-black/15" />
          </motion.div>
        </AnimatePresence>
      </div>
        ...
        <div className="max-w-2xl lg:max-w-3xl space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <button
              ...
              className="flex items-center gap-2.5 px-6 sm:px-8 py-3.5 rounded-full bg-[#2563EB] text-white font-medium text-sm sm:text-base tracking-tight hover:bg-[#1D4ED8] active:scale-95 transition-[background-color,transform] duration-150 shadow-[0_4px_16px_rgba(37,99,235,0.25)] group cursor-pointer"
            >
          ...
          <div className="pt-6 flex flex-wrap items-center gap-6 text-xs text-slate-600 border-t border-slate-200">
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-1" aria-label="5 star rating">
                {[1, 2, 3, 4, 5].map((i) => (
                  <Star key={i} className="w-3.5 h-3.5 fill-[#2563EB] text-[#2563EB]" aria-hidden="true" />
                ))}
              </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full mt-6 flex flex-col sm:flex-row items-end justify-between gap-4">
        ...
        <div className="hidden sm:flex items-center gap-3 px-4 py-2 rounded-2xl bg-white/85 backdrop-blur-md border border-slate-200 text-xs text-slate-700 shadow-xs">
          <span className="font-mono text-[#2563EB] font-bold">{activeSlide.category}</span>
              ...
              <button
                ...
                className={`group relative w-16 h-14 sm:w-20 sm:h-16 rounded-xl overflow-hidden transition-[transform,opacity,box-shadow] duration-200 cursor-pointer active:scale-95 ${
                  idx === activeSlideIndex
                    ? 'ring-2 ring-[#2563EB] scale-105 shadow-sm'
